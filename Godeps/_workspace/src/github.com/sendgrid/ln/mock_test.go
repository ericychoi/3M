...
func TestMockEvent(t *testing.T) {
	testLogger := NewMockLogger()
	testLogger.Event("test", Map{"foo": "bar"})
	event, err := testLogger.Find(EVENT_LOG_LEVEL, "", Map{
		"event": "test",
		"foo":   "bar",
	})
	if err != nil {
		t.Fatalf("got error trying to find event: %s", err)
	}
	if event.Message != "" {
		t.Errorf("got message %q, wanted empty string", event.Message)
	}
}
