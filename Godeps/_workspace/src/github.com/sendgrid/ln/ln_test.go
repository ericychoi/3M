...
func TestInvalidLogOutputFile(t *testing.T) {
	...
	defer func() {
		...
		if r := recover(); r != nil {
			...
			// Error message might be different on different OSes
			msg := "Initializing ln logger: could not create endpoint /thisisnotarealdirectory/output.log, error: open /thisisnotarealdirectory/output.log: no such file or directory"
			if r != msg {
				t.Errorf("should have panicked with:\n\t%q\n\tinstead got %s", msg, r)
			}
			return
		}
	...
	New("/thisisnotarealdirectory/output.log", "emerg", "auth", "test app")
