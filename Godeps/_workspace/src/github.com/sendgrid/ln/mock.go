...
// Find will return a message matching the given values, even if they are a subset
func (m *MockLogger) Find(p syslog.Priority, msg string, values Map) (message, error) {
	for _, logMessage := range m.tracking[p] {
		if msg != logMessage.Message {
			continue
		}

	INNER:
		for k, v := range values {
			if !reflect.DeepEqual(v, logMessage.Values[k]) {
				break INNER
			}
		}
